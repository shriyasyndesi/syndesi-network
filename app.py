...
st.set_page_config(page_title="Syndesi Concierge", page_icon="🔶", layout="tight")
...
<style>
    ...
    .user-msg-box {{
        ...
        margin-left: auto; max-width: 80%; color: #1F2937;
    }}
...
st.markdown(f"""
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 30px; border-bottom: 1px solid #E5E7EB; padding-bottom: 15px;">
        <img src="{LOGO_URL}" width="35" style="border-radius: 5px;">
        ...
        if isinstance(content, str):
            st.markdown(f"""
                <div class="chat-bubble-container">
                    <div class="avatar"><img src="{LOGO_URL}" width="20"></div>
                    <div class="msg-content">{content}</div>
                </div>
            """, unsafe_allow_html=True)
        # AI Result with Cards
            ...
            for m in res.get("matches", []):
                st.markdown(f"""
                    <div class="expert-card">
                        <span class="expert-tag">{m.get('confidence')}% Match</span>
                        ...
                        <a href="mailto:{m.get('email')}" class="connect-btn">Connect with Specialist</a>
                    </div>
                """, unsafe_allow_html=True)

# ── CHAT INPUT (THE SMART PART) ──
...
if st.session_state.messages[-1]["role"] == "user":
    with st.spinner("Searching Syndesi network..."):
        df, _ = load_professionals()
        summary = "\n".join([" | ".join(map(str, row)) for row in df.values])
        
        ai_prompt = f"User needs: '{st.session_state.messages[-1]['content']}'. Matches from: {summary}. Return ONLY JSON with 'reply' and 'matches' array."
        ...
        if not err:
            try:
                ...
                st.session_state.messages.append({"role": "assistant", "content": "I found some experts, but I'm having trouble displaying them. Try rephrasing?"})
        else:
            st.session_state.messages.append({"role": "assistant", "content": f"Sorry, I hit an error: {err}"})
    st.rerun()
